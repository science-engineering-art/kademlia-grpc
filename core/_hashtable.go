...
// TODO
// leandro_driguez: no se utiliza este método
func (ht *hashTable) removeNode(ID []byte) {
	...
	for i, v := range bucket {
		if bytes.Equal(v.ID, ID) {
			...
			// // leandro_driguez: por qué no detenerse ya?
			// break
		}
	}
...
func (ht *hashTable) getAllNodesInBucketCloserThan(bucket int, id []byte) [][]byte {
	...
	var nodes [][]byte
	for _, v := range b {
		d1 := ht.getDistance(id, ht.Self.ID) // leandro_driguez: se puede ejecutar fuera del ciclo
...
func (ht *hashTable) getRandomIDFromBucket(bucket int) []byte {
	...
	// TODO
	// leandro_driguez: el algoritmo funciona, pero es innecesario
	// ir construyendo el firstByte, se puede partir de diffferingBitStart
	// e ir generando el resto
	var firstByte byte
